...
void semisync_sender() {
    ...
    for (int replica_id = 0; replica_id < SLAVE_NODE_NUM; replica_id++) {
        ret = pthread_join(sender_thread[replica_id], NULL);
        if (ret != 0) {
            exit(1);
        }
    }
...
void *semisync_udp_sender(sender_thread_info_t *arg) {
    ...
    for (int i = 0; i < 10; i++) {
        sprintf(send_msg, "[msg]master->replica%d count%d", arg->connection_slave_id, i);
